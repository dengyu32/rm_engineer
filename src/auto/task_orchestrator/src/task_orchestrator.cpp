...
inline std::vector<double> toPoseVec(const engineer_interfaces::msg::Pose &pose) {
  return {pose.x, pose.y, pose.z, pose.qx, pose.qy, pose.qz, pose.qw};
}

inline std::vector<double> toVectorVec(const geometry_msgs::msg::Vector3 &vec) {
  return {vec.x, vec.y, vec.z};
}

inline Step withPostDelay(Step step, int delay_ms) {
  step.post_delay_ms = delay_ms;
  return step;
}
...
Step make_arm_move_pose(const char *id, const engineer_interfaces::msg::Pose &target,
  ...
  step.command.params["target_pose"] = toPoseVec(target);
  return step;
}
...
Step make_arm_move_joints(const char *id, const std::array<float, 6> &target,
                          int timeout_ms = 8000, int retries = 1) {
  Step step = makeStep(id, protocol::kArmMoveKind, timeout_ms, retries);
  step.command.params["target_joints"] =
...
Step make_arm_move_vector(const char *id, const geometry_msgs::msg::Vector3 &target,
                          int timeout_ms = 8000, int retries = 1) {
  Step step = makeStep(id, protocol::kArmMoveKind, timeout_ms, retries);
  step.command.params["target_vector"] = toVectorVec(target);
  return step;
}
...
Step make_arm_move_from_key(const char *id, const char *key_name, const char *param_name,
  ...
  step.bindings.push_back(bindDirect(taskKey(key_name), param_name));
  return step;
}
...
Step make_gripper(const char *id, const char *action) {
  ...
  if (std::string(action) == "open") {
    setParam(step.command, "action", "open");
  } else if (std::string(action) == "close") {
    setParam(step.command, "action", "close");
  } else {
    setParam(step.command, "action", "open");
  }
  return step;
...
Step make_slot_select(const char *id, const char *strategy) {
  ...
  if (std::string(strategy) == "put") {
    setParam(step.command, "strategy", "put");
  } else if (std::string(strategy) == "take") {
    setParam(step.command, "strategy", "take");
  } else {
    setParam(step.command, "strategy", "put");
  }
...
Step make_slot_lock(const char *id, const char *slot_key) {
  ...
  step.bindings.push_back(bindDirect(taskKey(slot_key), "slot_id"));
  return step;
}
...
core::TaskPlan make_auto_grab() {
  return TaskBuilder(TaskId::AUTO_GRAB)
      ...
      .add(make_arm_move_from_key("approach_to_grab",
                                  ...
                                  "target_pose"))
      .add(withPostDelay(make_gripper("gripper_close", "close"), 600))
...
core::TaskPlan make_auto_store() {
  return TaskBuilder(TaskId::AUTO_STORE)
      ...
      .add(withPostDelay(make_gripper("gripper_open", "open"), 600))
      .add(make_slot_lock("lock_slot", protocol::kSlotId))
      .add(make_arm_move_joints("move_home", task_orchestrator::preset::HOME))
      .build();
}

core::TaskPlan make_auto_get() {
...
core::TaskPlan make_fixed_grab() {
  ...
  return TaskBuilder(TaskId::FIXED_GRAB)
      .add(withPostDelay(make_gripper("open_gripper", "open"), 300))
...
core::TaskPlan make_test_cartesian() {
  const auto target0 =
      make_target(-0.63131, -0.052031, 0.67485, -0.18348, -0.64249, 0.27727, 0.6904);
  const auto target1 =
      make_target(-0.52816, 0.07035, 0.57682, -0.35088, -0.59813, 0.36457, 0.62146);
  ...
  return TaskBuilder(TaskId::TEST_CARTESIAN)
      ...
      .add(withPostDelay(make_gripper("gripper_close", "close"), 200))
