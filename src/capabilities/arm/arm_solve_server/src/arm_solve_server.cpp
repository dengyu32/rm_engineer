...
namespace {
std::shared_ptr<GoalContext>
makeGoalContext(const ArmMove::Goal &goal,
  ...
  ctx->req.target_vector = {static_cast<double>(goal.target_vector.x),
                            static_cast<double>(goal.target_vector.y),
                            static_cast<double>(goal.target_vector.z)};
  ctx->req.target_length = goal.target_length;
  ctx->req.target_joints.assign(goal.target_joints.begin(),
                                goal.target_joints.end());
...
std::shared_ptr<ArmMove::Result> make_move_result(bool success,
...
// 任务状态反馈
// ------------------------------------------------------------------
void finish_move_goal(const std::shared_ptr<GoalHandleArmMove> &gh,
                      bool success, bool canceled, const std::string &msg) {
  if (!gh) {
    return;
  }
...
ArmSolveServer::ArmSolveServer(const rclcpp::NodeOptions &options)
    : Node("arm_solve_server", rclcpp::NodeOptions(options)),
      logger_(this->get_logger()), config_(ArmSolveConfig::Load(*this)) {
  log_tools::init_console_logger("core");

  this->declare_parameter("joints_max_step_rad", 0.1);

  callback_handle_ = this->add_on_set_parameters_callback(std::bind(
      &ArmSolveServer::parameter_callback, this, std::placeholders::_1));
  ...
  // Delay SolveExecutor creation until the node has finished constructing and
  // all ROS interfaces are registered.
  init_timer_ = this->create_wall_timer(
      1ms, std::bind(&ArmSolveServer::initializeSolveExecutor, this));

  LOGI("[ARM_SOLVE_SERVER] started");
  RCLCPP_INFO(this->logger_, "%s", config_.summary().c_str());
}

void ArmSolveServer::initializeSolveExecutor() {
  {
    std::scoped_lock<std::mutex> lock(solve_executor_mutex_);
    if (solve_executor_) {
      if (init_timer_) {
        init_timer_->cancel();
        init_timer_.reset();
  ...
  std::unique_ptr<solve_executor::SolveExecutor> new_executor;
  try {
    new_executor = std::make_unique<solve_executor::SolveExecutor>(*this);
  } catch (const std::exception &e) {
    LOGE("[arm_solve_server] SolveExecutor initialization failed: {}", e.what());
    return;
  } catch (...) {
    LOGE("[arm_solve_server] SolveExecutor initialization failed: unknown "
         "exception");
    return;
  }

  {
    std::scoped_lock<std::mutex> lock(solve_executor_mutex_);
    if (!solve_executor_) {
      solve_executor_ = std::move(new_executor);
      LOGI("[arm_solve_server] SolveExecutor initialized");
    }
    if (solve_executor_ && init_timer_) {
      init_timer_->cancel();
      init_timer_.reset();
    }
  }
...
bool ArmSolveServer::isSolveExecutorReady() const {
  std::scoped_lock<std::mutex> lock(solve_executor_mutex_);
  return static_cast<bool>(solve_executor_);
}

rcl_interfaces::msg::SetParametersResult ArmSolveServer::parameter_callback(
  ...
  for (const auto &parameter : parameters) {
    ...
    if (name == "arm_action_name" || name == "joint_states_verbose_topic" ||
        name == "joint_cmd_topic" || name == "joint_names" ||
        name == "joint_count") {
      result.successful = false;
      result.reason =
          "runtime update is not supported for ArmSolveServer transport "
          "parameters";
      return result;
    }

    if (name == "joints_max_step_rad") {
      const auto type = parameter.get_type();
      if (type != rclcpp::ParameterType::PARAMETER_DOUBLE &&
          type != rclcpp::ParameterType::PARAMETER_INTEGER) {
        result.successful = false;
        result.reason = "joints_max_step_rad must be a numeric value";
        return result;
      }
      ...
      if (!std::isfinite(value) || value <= 0.0) {
        result.successful = false;
        result.reason = "joints_max_step_rad must be a finite positive value";
        return result;
      }

      std::scoped_lock<std::mutex> lock(solve_executor_mutex_);
      if (solve_executor_) {
        solve_executor_->updateJointsMaxStepRad(value);
      }
      LOGI("[arm_solve_server] Updated joints_max_step_rad to {}", value);
    }

    if (name == "nominal_joint_speed") {
      const auto type = parameter.get_type();
      if (type != rclcpp::ParameterType::PARAMETER_DOUBLE &&
          type != rclcpp::ParameterType::PARAMETER_INTEGER) {
        result.successful = false;
        result.reason = "nominal_joint_speed must be a numeric value";
        return result;
      }
      ...
      if (!std::isfinite(value) || value <= 0.0) {
        result.successful = false;
        result.reason = "nominal_joint_speed must be a finite positive value";
        return result;
      }

      std::scoped_lock<std::mutex> lock(solve_executor_mutex_);
      if (solve_executor_) {
        solve_executor_->updateNominalJointSpeed(value);
      }
      LOGI("[arm_solve_server] Updated nominal_joint_speed to {}", value);
    }
  }
...
ArmSolveServer::handle_cancel(const std::shared_ptr<GoalHandleArmMove> gh) {
  ...
  if (active && active.get() == gh.get() && active_ctx_) {
    ...
    std::scoped_lock<std::mutex> executor_lock(solve_executor_mutex_);
    if (solve_executor_) {
...
void ArmSolveServer::handle_accepted(
    const std::shared_ptr<GoalHandleArmMove> gh) {
...
void ArmSolveServer::execute(const std::shared_ptr<GoalHandleArmMove> gh,
  ...
  try {
    ...
    bool executor_ok = false;
    {
      std::scoped_lock<std::mutex> executor_lock(solve_executor_mutex_);
      executor_ok =
          solve_executor_ && solve_executor_->execute(ctx->req, traj_msg, err);
    }
    if (!executor_ok) {
      if (!isSolveExecutorReady() && err.empty()) {
...
bool ArmSolveServer::publishTrajectoryPoints(
  ...
  for (size_t i = 0; i < traj.points.size(); ++i) {
    ...
    if (traj.points[i].positions.size() < traj.joint_names.size()) {
      LOGE("[arm_solve_server] Trajectory point {} positions size {} < "
           "joint_names size {}",
