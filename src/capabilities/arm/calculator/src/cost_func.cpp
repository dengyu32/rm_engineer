...
{
//雅可比矩阵条件数计算代价
double jacobian_condition(moveit::core::RobotState& st, const moveit::core::JointModelGroup* jmg,
...
CostFunc::CostFunc(const moveit::core::RobotState& reference_state,
                   const moveit::core::JointModelGroup* joint_model_group, const moveit::core::LinkModel* link_model,
                   const CostOptions& options)
  : reference_state_(reference_state), joint_model_group_(joint_model_group), link_model_(link_model), options_(options)
{
}

double CostFunc::compute(const std::vector<double>& q_from, const std::vector<double>& q_to) const
{
  if (!joint_model_group_ || !link_model_)
  {
    LOGE("[solve_executor][cost_func] Invalid model handles: joint_model_group or link_model is null");
  ...
  const double cond_penalty =
      (cond > options_.hard_condition_threshold) ? options_.hard_penalty : (cond * options_.condition_weight);
