...
//   // --- 新增逻辑：将解集按照与 seed_state (q_base) 的连续性排序 ---
//   std::stable_sort(solutions.begin(), solutions.end(),
//     [&q_base](const std::vector<double>& a, const std::vector<double>& b) {
//       double dist_a = 0.0;
//       double dist_b = 0.0;
//       for (size_t i = 0; i < q_base.size(); ++i)
//       {
//         double diff_a = a[i] - q_base[i];
//         double diff_b = b[i] - q_base[i];
//         dist_a += diff_a * diff_a;
//         dist_b += diff_b * diff_b;
//       }
//       return dist_a < dist_b; // 升序排列，L2距离最小的（最连续的）排在最前面
//     });
