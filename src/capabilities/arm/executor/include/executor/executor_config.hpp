                             ...
                             public params_utils::JointResetConfig
{
  ...
  static SolveExecutorConfig Load(rclcpp::Node& node)
  {
    ...
    declare_get_checked(
        ...
        "must be a positive number");
    declare_get_checked(
        node, "max_acc_scaling", cfg.max_acc_scaling, [](const double v) { return v > 0; },
        "must be a positive number");
    declare_get(node, "use_vision_target_vector", cfg.use_vision_target_vector);
    declare_get_checked(
        node, "sample_step_m", cfg.sample_step_m, [](const double v) { return v > 0; },
        "must be a positive number");
    ...
    declare_get_checked(node, "max_contacts_per_pair", cfg.max_contacts_per_pair, [](const double v) { return v > 0; },
                        "must be a positive number");
    declare_get_checked(
        node, "joints_max_step_rad", cfg.joints_max_step_rad, [](const double v) { return v > 0; },
        "must be a positive number");
