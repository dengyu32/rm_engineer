...
std::optional<Trajectory> LimitPlanner::plan(const moveit::core::JointModelGroup* jmg_, const std::string ee_link_,
                                             ...
                                             std::vector<std::vector<double>>* joint_path_out)
{
  if (!adapter_)
  {
    ...
    LOGE("[solve_executor][limit_planner] {}", err);
    return std::nullopt;
  }
  if (!jmg_)
  {
  ...
  if (sam_configs.enable_target_pose_sampling && sam_configs.sampling_mode == SamplingMode::ROLL_SAMPLE)
  {
    ...
    evaluate_candidates_with_ik(candidates, model, jmg_->getName(), ee_link_, start_state, current_joints, ik_opt,
                                sam_configs);

    const auto best_it = std::find_if(candidates.begin(), candidates.end(), [](const PoseSampleCandidate& c) {
  ...
  else
  {
    ...
    if (!ik_ok)
    {
      const Eigen::Isometry3d current_fk = start_state.getGlobalLinkTransform(ee_link_);
      ik_ok = ik_state.setFromIK(jmg_, current_fk, ee_link_,
                                 2.0);  // 如果目标位姿不可达，尝试用当前位姿作为目标再次求解，确保至少有一个解
      if (!ik_ok)
      {
        LOGE("[solve_executor][limit_planner] IK failed");
        err = "IK failed";
        return std::nullopt;
      }
    }
  ...
  {
    LOGE("[solve_executor][limit_planner] IK variables size mismatch");
  ...
  if (!plan_res)
  {
    LOGE("[solve_executor] plan_to_joint_target failed");
