...
inline void drawBoundingBox(cv::Mat &image, const BoundingBox &box,
                            const std::string &label, const cv::Scalar &color,
                            int thickness = 2) {
  // Draw rectangle
  cv::rectangle(image, cv::Point(box.x, box.y),
                cv::Point(box.x + box.width, box.y + box.height), color,
  ...
  if (!label.empty()) {
    ...
    cv::Size textSize =
        cv::getTextSize(label, fontFace, fontScale, textThickness, &baseline);

    int labelY = std::max(box.y, textSize.height + 5);
    cv::Point labelTopLeft(box.x, labelY - textSize.height - 5);
    cv::Point labelBottomRight(box.x + textSize.width + 5,
                               labelY + baseline - 5);

    cv::rectangle(image, labelTopLeft, labelBottomRight, color, cv::FILLED);
    cv::putText(image, label, cv::Point(box.x + 2, labelY - 2), fontFace,
...
inline void drawBoundingBoxWithMask(cv::Mat &image, const BoundingBox &box,
                                    const std::string &label,
                                    const cv::Scalar &color,
                                    float maskAlpha = 0.4f) {
  // Draw semi-transparent fill
  cv::Mat overlay = image.clone();
  cv::rectangle(overlay, cv::Rect(box.x, box.y, box.width, box.height), color,
                cv::FILLED);
...
inline void drawOrientedBoundingBox(cv::Mat &image,
  ...
  cv::RotatedRect rotatedRect(cv::Point2f(obb.x, obb.y),
  ...
  if (!label.empty()) {
    ...
    cv::putText(image, label, cv::Point(x, y), fontFace, fontScale,
                cv::Scalar::all(255), textThickness, cv::LINE_AA);
  }
}
...
inline void drawPoseSkeleton(cv::Mat &image,
  ...
  static const std::vector<int> limbColorIndices = {
      9, 9, 9, 9, 7, 7, 7, 0, 0, 0, 0, 0, 16, 16, 16, 16, 16, 16};
  ...
  for (size_t i = 0; i < keypoints.size(); ++i) {
    if (keypoints[i].confidence >= kptThreshold) {
      ...
      cv::circle(image, cv::Point(x, y), kptRadius, palette[colorIdx], -1,
                 cv::LINE_AA);
    }
  }
  ...
  for (size_t j = 0; j < skeleton.size(); ++j) {
    ...
    if (src < static_cast<int>(keypoints.size()) &&
        dst < static_cast<int>(keypoints.size()) && valid[src] && valid[dst]) {
      int limbColorIdx =
          (j < limbColorIndices.size()) ? limbColorIndices[j] : 0;
      cv::line(image, kptPoints[src], kptPoints[dst], palette[limbColorIdx],
...
inline void drawSegmentationMask(cv::Mat &image, const cv::Mat &mask,
