 ...
 * - 热插拔使用 libusb_hotplug 机制检测断开并配合自动重连。
 * - 内部完成流式拼帧，并将完整数据包交给上层回调。
 */
 ...
 * 回调逻辑：
 * 1. 传输状态检查（若失败则认为设备断开）
 * 2. 若收到有效数据，则交给 Device::Impl::parse() 拼帧
...
void Device::Impl::alloc_transfer() {
  ...
  libusb_fill_bulk_transfer(
      ...
      [](libusb_transfer *tr) {
        ...
        if (len > 0) {
          self->parse(reinterpret_cast<std::byte *>(tr->buffer),
 ...
 *
 * 说明：
 * - 使用同步 bulk 传输；若 USB 资源锁正忙，直接跳过本周期发送
 * - 对于高频发送，可扩展为异步发送队列
 */
bool Device::Impl::sync_send(uint8_t *data, std::size_t size,
  ...
  if (usb_ms > 5) {
    RCLCPP_WARN(rclcpp::get_logger("usb_cdc"),
                "[send][split] usb_send=%ld ms", usb_ms);
  }
...
void Device::set_packet_callback(uint8_t id, PacketCallback callback) {
  impl_->set_packet_callback(id, std::move(callback));
}
...
void Device::Impl::parse(const std::byte *data, size_t size) {
  ...
  while (true) {
    auto sof_it = std::find(stream_buffer_.begin(), stream_buffer_.end(),
                            std::byte{HeaderFrame::SoF()});
    if (sof_it == stream_buffer_.end()) {
      stream_buffer_.clear();
      return;
    }

    if (sof_it != stream_buffer_.begin()) {
      stream_buffer_.erase(stream_buffer_.begin(), sof_it);
    }

    if (stream_buffer_.size() < sizeof(HeaderFrame)) {
      return;
    }

    HeaderFrame header_frame;
    std::memcpy(&header_frame, stream_buffer_.data(), sizeof(HeaderFrame));

    const size_t expected_size =
        sizeof(HeaderFrame) + header_frame.len + sizeof(uint8_t);
    if (stream_buffer_.size() < expected_size) {
      return;
    }

    const uint8_t eof =
        std::to_integer<uint8_t>(stream_buffer_[expected_size - 1]);
    if (eof != HeaderFrame::EoF()) {
      RCLCPP_WARN_THROTTLE(rclcpp::get_logger("usb_cdc"),
                           *clock, 2000,
