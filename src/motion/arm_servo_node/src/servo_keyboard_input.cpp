// ============================================================================
//  servo_keyboard_input.cpp
//  - '+' / '=' : speed up
//  - '-' / '_' : speed down
//  - '0'       : reset speed to 1.0x
//  - Prints current speed scales
// ============================================================================

#include <control_msgs/msg/joint_jog.hpp>
#include <geometry_msgs/msg/twist_stamped.hpp>
#include <rclcpp/rclcpp.hpp>

#include <signal.h>
#include <stdio.h>
#include <termios.h>
#include <unistd.h>

#include <algorithm>
#include <cstring>
#include <functional>
#include <thread>

// ----------------------------------------------------------------------------
// Define used keys
// ----------------------------------------------------------------------------
#define KEYCODE_RIGHT 0x43
#define KEYCODE_LEFT 0x44
#define KEYCODE_UP 0x41
#define KEYCODE_DOWN 0x42
#define KEYCODE_PERIOD 0x2E
#define KEYCODE_SEMICOLON 0x3B
#define KEYCODE_0 0x30
#define KEYCODE_1 0x31
#define KEYCODE_2 0x32
#define KEYCODE_3 0x33
#define KEYCODE_4 0x34
#define KEYCODE_5 0x35
#define KEYCODE_6 0x36
#define KEYCODE_7 0x37
#define KEYCODE_Q 0x71
#define KEYCODE_W 0x77
#define KEYCODE_E 0x65
#define KEYCODE_R 0x72

// Speed keys
#define KEYCODE_PLUS 0x2B       // '+'
#define KEYCODE_EQUALS 0x3D     // '='  (some terminals report Shift+= as '=')
#define KEYCODE_MINUS 0x2D      // '-'
#define KEYCODE_UNDERSCORE 0x5F // '_'  (Shift+-)

// ----------------------------------------------------------------------------
// Some constants used in the Servo Teleop
// ----------------------------------------------------------------------------
const std::string TWIST_TOPIC = "/servo_node/delta_twist_cmds";
const std::string JOINT_TOPIC = "/servo_node/delta_joint_cmds";
const size_t ROS_QUEUE_SIZE = 10;
const std::string EEF_FRAME_ID = "link6";
const std::string BASE_FRAME_ID = "base_link";
// ============================================================================
//  KeyboardReader
// ============================================================================
class KeyboardReader {
public:
  KeyboardReader() : kfd(0) {
    // get the console in raw mode
    tcgetattr(kfd, &cooked);
    struct termios raw;
    memcpy(&raw, &cooked, sizeof(struct termios));
    raw.c_lflag &= ~(ICANON | ECHO);
    // Setting a new line, then end of file
    raw.c_cc[VEOL] = 1;
    raw.c_cc[VEOF] = 2;
    tcsetattr(kfd, TCSANOW, &raw);
  }

  void readOne(char *c) {
    int rc = read(kfd, c, 1);
    if (rc < 0) {
      throw std::runtime_error("read failed");
    }
  }

  void shutdown() { tcsetattr(kfd, TCSANOW, &cooked); }

private:
  int kfd;
  struct termios cooked;
};

// ============================================================================
//  KeyboardServo
// ============================================================================
class KeyboardServo {
public:
  KeyboardServo();
  int keyLoop();

private:
  void spin();

  // --------------------------------------------------------------------------
  // Speed scaling helpers
  // --------------------------------------------------------------------------
  void clampSpeed();
  void printSpeed() const;

  rclcpp::Node::SharedPtr nh_;

  rclcpp::Publisher<geometry_msgs::msg::TwistStamped>::SharedPtr twist_pub_;
  rclcpp::Publisher<control_msgs::msg::JointJog>::SharedPtr joint_pub_;

  std::string frame_to_publish_;

  // Base joint velocity (direction is controlled by sign)
  double joint_vel_cmd_;

  // Speed scale multipliers
  double twist_scale_{1.0};
  double joint_scale_{1.0};

  // Step and clamp ranges
  double step_{1.25};
  double min_scale_{0.05};
  double max_scale_{5.0};
};

KeyboardServo::KeyboardServo()
    : frame_to_publish_(BASE_FRAME_ID), joint_vel_cmd_(2.0) {
  nh_ = rclcpp::Node::make_shared("servo_keyboard_input");

  twist_pub_ = nh_->create_publisher<geometry_msgs::msg::TwistStamped>(
      TWIST_TOPIC, ROS_QUEUE_SIZE);
  joint_pub_ = nh_->create_publisher<control_msgs::msg::JointJog>(
      JOINT_TOPIC, ROS_QUEUE_SIZE);

  printSpeed();
}

void KeyboardServo::clampSpeed() {
  twist_scale_ = std::min(std::max(twist_scale_, min_scale_), max_scale_);
  joint_scale_ = std::min(std::max(joint_scale_, min_scale_), max_scale_);
}

void KeyboardServo::printSpeed() const {
  RCLCPP_INFO(
      nh_->get_logger(),
      "[speed] twist_scale=%.2fx, joint_scale=%.2fx, base_joint_vel=%.2f",
      twist_scale_, joint_scale_, joint_vel_cmd_);
}

KeyboardReader input;

void quit(int sig) {
  (void)sig;
  input.shutdown();
  rclcpp::shutdown();
  exit(0);
}

int main(int argc, char **argv) {
  rclcpp::init(argc, argv);
  KeyboardServo keyboard_servo;

  signal(SIGINT, quit);

  int rc = keyboard_servo.keyLoop();
  input.shutdown();
  rclcpp::shutdown();

  return rc;
}

void KeyboardServo::spin() {
  while (rclcpp::ok()) {
    rclcpp::spin_some(nh_);
  }
}

int KeyboardServo::keyLoop() {
  char c;
  bool publish_twist = false;
  bool publish_joint = false;

  std::thread{std::bind(&KeyboardServo::spin, this)}.detach();

  puts("Reading from keyboard");
  puts("---------------------------");
  puts("Use arrow keys and the '.' and ';' keys to Cartesian jog");
  puts("Use 'W' to Cartesian jog in the world frame, and 'E' for the "
       "End-Effector frame");
  puts("Use 1|2|3|4|5|6 keys to joint jog. 'R' to reverse the direction of "
       "jogging.");
  puts("Use '+'/'=' to speed up, '-'/'_' to slow down, '0' to reset to 1.0x.");
  puts("'Q' to quit.");

  for (;;) {
    // get the next event from the keyboard
    try {
      input.readOne(&c);
    } catch (const std::runtime_error &) {
      perror("read():");
      return -1;
    }

    RCLCPP_DEBUG(nh_->get_logger(), "value: 0x%02X\n",
                 static_cast<unsigned char>(c));

    // Create the messages we might publish
    auto twist_msg = std::make_unique<geometry_msgs::msg::TwistStamped>();
    auto joint_msg = std::make_unique<control_msgs::msg::JointJog>();

    // Use read key-press
    switch (c) {
    // ----------------------------------------------------------------------
    // Speed control
    // ----------------------------------------------------------------------
    case KEYCODE_PLUS:
    case KEYCODE_EQUALS:
      twist_scale_ *= step_;
      joint_scale_ *= step_;
      clampSpeed();
      printSpeed();
      break;

    case KEYCODE_MINUS:
    case KEYCODE_UNDERSCORE:
      twist_scale_ /= step_;
      joint_scale_ /= step_;
      clampSpeed();
      printSpeed();
      break;

    case KEYCODE_0:
      twist_scale_ = 1.0;
      joint_scale_ = 1.0;
      printSpeed();
      break;

    // ----------------------------------------------------------------------
    // Cartesian jog
    // ----------------------------------------------------------------------
    case KEYCODE_LEFT:
      RCLCPP_DEBUG(nh_->get_logger(), "LEFT");
      twist_msg->twist.linear.y = -1.0 * twist_scale_;
      publish_twist = true;
      break;
    case KEYCODE_RIGHT:
      RCLCPP_DEBUG(nh_->get_logger(), "RIGHT");
      twist_msg->twist.linear.y = 1.0 * twist_scale_;
      publish_twist = true;
      break;
    case KEYCODE_UP:
      RCLCPP_DEBUG(nh_->get_logger(), "UP");
      twist_msg->twist.linear.x = 1.0 * twist_scale_;
      publish_twist = true;
      break;
    case KEYCODE_DOWN:
      RCLCPP_DEBUG(nh_->get_logger(), "DOWN");
      twist_msg->twist.linear.x = -1.0 * twist_scale_;
      publish_twist = true;
      break;
    case KEYCODE_PERIOD:
      RCLCPP_DEBUG(nh_->get_logger(), "PERIOD");
      twist_msg->twist.linear.z = -1.0 * twist_scale_;
      publish_twist = true;
      break;
    case KEYCODE_SEMICOLON:
      RCLCPP_DEBUG(nh_->get_logger(), "SEMICOLON");
      twist_msg->twist.linear.z = 1.0 * twist_scale_;
      publish_twist = true;
      break;

    case KEYCODE_E:
      RCLCPP_DEBUG(nh_->get_logger(), "E");
      frame_to_publish_ = EEF_FRAME_ID;
      break;
    case KEYCODE_W:
      RCLCPP_DEBUG(nh_->get_logger(), "W");
      frame_to_publish_ = BASE_FRAME_ID;
      break;

    // ----------------------------------------------------------------------
    // Joint jog
    // ----------------------------------------------------------------------
    case KEYCODE_1:
      RCLCPP_DEBUG(nh_->get_logger(), "1");
      joint_msg->joint_names.push_back("joint1");
      joint_msg->velocities.push_back(joint_vel_cmd_ * joint_scale_);
      publish_joint = true;
      break;
    case KEYCODE_2:
      RCLCPP_DEBUG(nh_->get_logger(), "2");
      joint_msg->joint_names.push_back("joint2");
      joint_msg->velocities.push_back(joint_vel_cmd_ * joint_scale_);
      publish_joint = true;
      break;
    case KEYCODE_3:
      RCLCPP_DEBUG(nh_->get_logger(), "3");
      joint_msg->joint_names.push_back("joint3");
      joint_msg->velocities.push_back(joint_vel_cmd_ * joint_scale_);
      publish_joint = true;
      break;
    case KEYCODE_4:
      RCLCPP_DEBUG(nh_->get_logger(), "4");
      joint_msg->joint_names.push_back("joint4");
      joint_msg->velocities.push_back(joint_vel_cmd_ * joint_scale_);
      publish_joint = true;
      break;
    case KEYCODE_5:
      RCLCPP_DEBUG(nh_->get_logger(), "5");
      joint_msg->joint_names.push_back("joint5");
      joint_msg->velocities.push_back(joint_vel_cmd_ * joint_scale_);
      publish_joint = true;
      break;
    case KEYCODE_6:
      RCLCPP_DEBUG(nh_->get_logger(), "6");
      joint_msg->joint_names.push_back("joint6");
      joint_msg->velocities.push_back(joint_vel_cmd_ * joint_scale_);
      publish_joint = true;
      break;

    case KEYCODE_R:
      RCLCPP_DEBUG(nh_->get_logger(), "R");
      joint_vel_cmd_ *= -1;
      printSpeed();
      break;

    case KEYCODE_Q:
      RCLCPP_DEBUG(nh_->get_logger(), "quit");
      return 0;

    default:
      break;
    }

    // If a key requiring a publish was pressed, publish the message now
    if (publish_twist) {
      twist_msg->header.stamp = nh_->now();
      twist_msg->header.frame_id = frame_to_publish_;

      RCLCPP_INFO_THROTTLE(
          nh_->get_logger(), *nh_->get_clock(), 500 /*ms*/,
          "[pub twist] frame=%s lin=(%.2f, %.2f, %.2f) scale=%.2fx",
          twist_msg->header.frame_id.c_str(), twist_msg->twist.linear.x,
          twist_msg->twist.linear.y, twist_msg->twist.linear.z, twist_scale_);

      twist_pub_->publish(std::move(twist_msg));
      publish_twist = false;
    } else if (publish_joint) {
      joint_msg->header.stamp = nh_->now();
      joint_msg->header.frame_id = BASE_FRAME_ID;
      joint_msg->duration = 0.1;

      if (!joint_msg->joint_names.empty() && !joint_msg->velocities.empty()) {
        RCLCPP_INFO_THROTTLE(nh_->get_logger(), *nh_->get_clock(), 500 /*ms*/,
                             "[pub joint] %s vel=%.2f scale=%.2fx",
                             joint_msg->joint_names[0].c_str(),
                             joint_msg->velocities[0], joint_scale_);
      }

      joint_pub_->publish(std::move(joint_msg));
      publish_joint = false;
    }
  }

  return 0;
}
