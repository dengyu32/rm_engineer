...
SolveExecutor::SolveExecutor(rclcpp::Node& host_node, const SolveExecutorConfig& config,
  ...
  , moveit_adapter_(std::shared_ptr<solve_core::MoveItAdapter>(this, [](solve_core::MoveItAdapter*) {}))
{
  config_.validate();
...
void SolveExecutor::lateInit(const std::shared_ptr<rclcpp::Node>& host_node)
{
  ...
  if (!move_group_)
  {
    move_group_ = std::make_unique<moveit::planning_interface::MoveGroupInterface>(host_node, config_.group_name);
  }
  if (!psm_)
  {
    psm_ = std::make_shared<planning_scene_monitor::PlanningSceneMonitor>(host_node, "robot_description");
...
bool SolveExecutor::execute(const arm_solve::GoalContext& ctx, solve_core::Trajectory& out_traj, std::string& err)
{
...
bool SolveExecutor::planTrajectory(const arm_solve::GoalContext& ctx, solve_core::Trajectory& out_traj, std::string& err)
{
  ...
  LOGI("[solve_executor] Planning successful");
  return true;
}
...
const moveit::core::JointModelGroup* SolveExecutor::joint_model_group(const std::string& group_name) const
{
...
std::string SolveExecutor::group_name() const
{
  return config_.group_name;
}
...
std::optional<solve_core::Trajectory> SolveExecutor::plan_to_joint_target(const std::vector<std::string>& joint_names,
                                                                          ...
                                                                          const solve_core::PlannerConfigs& options)
{
  ...
  for (const auto& pt : jt.points)
  {
    ...
    p.time_from_start =
        static_cast<double>(pt.time_from_start.sec) + static_cast<double>(pt.time_from_start.nanosec) * 1e-9;
    traj.points.push_back(std::move(p));
  }
  return traj;
}

bool SolveExecutor::check_self_collision(const moveit::core::RobotState& state, const std::string& group_name,
...
bool SolveExecutor::time_parameterize_trajectory(const moveit::core::RobotState& start_state,
                                                 const std::string& group_name, solve_core::Trajectory& traj,
                                                 double velocity_scaling, double accel_scaling, std::string& err) const
{
  if (!move_group_)
  {
