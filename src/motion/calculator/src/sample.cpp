...
std::vector<PoseSampleCandidate> generate_roll_samples(const Pose& base_pose, const SamplingConfigs& opt)
{
  ...
  const double roll_step = (roll_n > 1) ? (roll_max - roll_min) / static_cast<double>(roll_n - 1) : 0.0;
  ...
  for (int i = 0; i < roll_n; ++i)
  {
    ...
    Eigen::Quaterniond q_sample = Eigen::AngleAxisd(base_yaw, Eigen::Vector3d::UnitZ()) *
...
//只针对单个解进行评估，计算总代价
double evaluate_candidate_cost(PoseSampleCandidate& candidate, const std::vector<double>& current_joints,
                               const SamplingConfigs& opt)
{
  ...
  candidate.total_cost = opt.orientation_weight * candidate.orientation_cost +
                         opt.ik_distance_weight * candidate.ik_cost + opt.joint_motion_weight * motion;
  return candidate.total_cost;
}
...
void evaluate_candidates_with_ik(std::vector<PoseSampleCandidate>& candidates,
                                 ...
                                 const std::string& ee_link, const moveit::core::RobotState& seed_state,
                                 const std::vector<double>& current_joints, const IKOptions& ik_opt,
                                 const SamplingConfigs& opt)
{
  if (candidates.empty())
  {
    LOGE("[solve_executor][sample] Empty candidates input");
    return;
  }
  if (!robot_model)
  {
    LOGE("[solve_executor][sample] Robot model is null");
    return;
  }
  if (group_name.empty())
  {
    LOGE("[solve_executor][sample] Planning group is empty");
    return;
  }
  if (ee_link.empty())
  {
    LOGE("[solve_executor][sample] End-effector link is empty");
    return;
  }
  if (current_joints.empty())
  {
    LOGE("[solve_executor][sample] Current joints are empty");
    return;
  }
  ...
  if (valid_count == 0)
  {
    LOGE("[solve_executor][sample] IK evaluation finished with zero valid "
         "candidates");
  }
...
void select_best_candidates(std::vector<PoseSampleCandidate>& candidates, int top_k)
{
  ...
  std::stable_sort(candidates.begin(), candidates.end(), [](const PoseSampleCandidate& a, const PoseSampleCandidate& b) {
