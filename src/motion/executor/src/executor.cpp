                                ...
                                const collision::SelfCollisionDetector* self_collision_detector)
{
 ...
 if (!fill_joint_state_require_all(req.current_joints, jmg, start_state, err))
  ...
  const auto count = std::min(config_.joint_names.size(), req.target_joints.size());
  for (std::size_t i = 0; i < count; ++i)
  {
    targets.emplace(config_.joint_names[i], req.target_joints[i]);
  }

  if (targets.empty())
  {
    err = "Joint target is empty";
    LOGE("[solve_executor] {}", err);
    return false;
  }
  ...
  {
    err = "setJointValueTarget failed";
    LOGE("[solve_executor] {}", err);
    return false;
  }
  ...
  if (!ok)
  {
    err = "MoveGroup joint planning failed";
    LOGE("[solve_executor] {}", err);
    return false;
  }
