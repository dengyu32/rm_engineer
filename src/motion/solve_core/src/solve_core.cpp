...
bool fill_joint_state_allow_missing(const JointState& js, const moveit::core::JointModelGroup* jmg,
...
// 构造函数，初始化日志
SolveCore::SolveCore(std::shared_ptr<MoveItAdapter> adapter, const SolveCoreConfig& config)
...
// 直线规划
std::optional<SolveResponse> SolveCore::plan_cartesian(const SolveRequest& req, std::string& err)
{
  ...
  if (!traj)
  {
    LOGE("[solve_core] No Result from StraightPlanner");
    return std::nullopt;
    }
  ...
  LOGI("[solve_core][plan_cartesian] Planning successful with StraightPlanner,trajiectory will be returned!");
  return resp;
}
