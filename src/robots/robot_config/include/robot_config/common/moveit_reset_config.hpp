...
struct MoveItResetConfig
{
  ...
  static void Load(rclcpp::Node& node, MoveItResetConfig& cfg)
  {
    ...
    declare_get_checked(
        node, "group_name", cfg.group_name, [](const std::string& v) { return !v.empty(); }, "must not be empty");
    declare_get_checked(
        node, "ee_link_name", cfg.ee_link_name, [](const std::string& v) { return !v.empty(); }, "must not be empty");
    declare_get_checked(
        node, "planning_frame_id", cfg.planning_frame_id, [](const std::string& v) { return !v.empty(); },
        "must not be empty");
    cfg.validate();
  }
  ...
  std::string summary() const
  {
    ...
    oss << "   - planning_frame_id       : " << planning_frame_id << "\n";
    return oss.str();
  }
};
