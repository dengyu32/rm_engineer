...
#include "robot_config/core/params.hpp"
...
struct SolveExecutorResetConfig
{
  ...
  double nominal_joint_speed{ 1.5 };
  double min_dt{ 0.01 };
  ...
  double joints_max_step_rad{ 0.1 };
  ...
  static void Load(rclcpp::Node& node, SolveExecutorResetConfig& cfg)
  {
    ...
    declare_get_checked(
        node, "nominal_joint_speed", cfg.nominal_joint_speed,
        [](const double v) { return v > 0; }, "must be a positive number");
    declare_get_checked(
        node, "min_dt", cfg.min_dt,
        [](const double v) { return v > 0; }, "must be a positive number");
    ...
    declare_get_checked(node, "max_contacts_per_pair", cfg.max_contacts_per_pair, [](const double v) { return v > 0; },
                        "must be a positive number");
    declare_get_checked(
        node, "joints_max_step_rad", cfg.joints_max_step_rad, [](const double v) { return v > 0; }, "must be a positive number");
    cfg.validate();
  }
  ...
  std::string summary() const
  {
    ...
    oss << "   - nominal_joint_speed             : " << nominal_joint_speed << "\n";
    oss << "   - min_dt                          : " << min_dt << "\n";
