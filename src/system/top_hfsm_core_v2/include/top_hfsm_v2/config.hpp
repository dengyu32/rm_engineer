...
inline ArmSolveClientConfig ArmSolveClientConfig::Load(rclcpp::Node &node) {
  ...
  auto declare_get_checked = [&](const std::string &name, auto &value,
  ...
  declare_get_checked("arm_server_wait_ms", cfg.arm_server_wait_ms,
                      [](int v) { return v >= 100 && v <= 10000; },
                      "must be in [100, 10000]");
  cfg.validate();
  return cfg;
}
...
inline void ArmSolveClientConfig::validate() const {
  if (arm_action_name.empty()) {
    throw std::runtime_error(
        "ArmSolveClientConfig: arm_action_name must not be empty");
  }
}
...
inline void MoveItServoClientConfig::validate() const {
  if (!detail::not_empty(servo_start_service) ||
    ...
    throw std::runtime_error(
        "MoveItServoClientConfig: all service names must be non-empty");
  }
  if (!detail::not_empty(delta_twist_topic) ||
      !detail::not_empty(delta_joint_topic) ||
      !detail::not_empty(joint_states_custom_topic) ||
      !detail::not_empty(joint_states_verbose_topic)) {
    throw std::runtime_error(
        "MoveItServoClientConfig: all topic names must be non-empty");
  }
}
...
inline void GripperConfig::validate() const {
  ...
  if (gripper_publish_period_ms < 1 || gripper_publish_period_ms > 1000) {
    throw std::runtime_error(
        "GripperConfig: gripper_publish_period_ms must be in [1, 1000]");
  }
}
...
inline void TopHFSMConfig::validate() const {
  if (intent_cmd_topic.empty()) {
    throw std::runtime_error("TopHFSMConfig: intent_cmd_topic must not be empty");
  }
  if (intent_fb_topic.empty()) {
    throw std::runtime_error("TopHFSMConfig: intent_fb_topic must not be empty");
  }
  if (hfsm_update_period_ms < 1 || hfsm_update_period_ms > 1000) {
    throw std::runtime_error(
        "TopHFSMConfig: hfsm_update_period_ms must be in [1, 1000]");
  }
}
